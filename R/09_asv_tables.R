...
shared_asv_tables <- purrr::imap(shared_asvs_by_threshold, function(sharing_data, threshold_name) {
  ...
  gt_table <- tax_summary %>%
    ...
    tab_style(
      ...
      locations = cells_body(rows = seq(2, n(), 2))
    ) %>%
    cols_width(
      asv ~ px(150),
      kingdom ~ px(100),
  ...
  gtsave(
    gt_table,
    filename = paste0("data/output/figures/shared_asvs_", threshold_name, "_taxonomy_table.html")
  )

  cat("Created table for", threshold_name, ":", nrow(tax_summary), "shared ASVs\n")
...
shared_2plus_tables <- purrr::imap(shared_asvs_by_threshold, function(sharing_data, threshold_name) {
  ...
  gtsave(
    gt_table,
    filename = paste0("data/output/figures/shared_asvs_2plus_", threshold_name, "_taxonomy_table.html")
  )

  cat("Created 2+ table for", threshold_name, ":", nrow(tax_summary), "ASVs\n")
...
summary_data <- purrr::imap_dfr(shared_asvs_by_threshold, function(sharing_data, threshold_name) {
  tibble(
