...
aligned_matrices <- purrr::map(sites, function(s) {
  extract_matrices <- function(ps) {
    ...
    if (taxa_are_rows(ps)) {
      #  with samples on rows and features/OTUs in columns
      mat <- t(mat)
    }
    mat
  }
...
fits <- purrr::imap(aligned_matrices, function(mat_list, s) {
  cat(" Site:", s, "\n")
  ...
  purrr::map(methods, function(method) {
    ...
    purrr::map(matrices, function(matx) {
      run_spiec(
        mat_list[[matx]],
        method = method,
        rep.num = 50,
      ...
      set_names(c("bact_only", "fungi_only", "joint_kingdom"))
  }) |>
    set_names(c("mb", "glasso"))
})
...
network_plots <- purrr::map(sites, function(s) {
  list(
    mb = list(
      bact_only = plot_igraph(
        ...
        mode = "maxabs",
        label = NULL,
        main_title = paste(s, "Bacteria-only (MB)"),
        kingdom = c("Bacteria")
      ),
      fungi_only = plot_igraph(
        fit = fits[[s]]$mb$fungi_only,
        aligned_matrix1 = aligned_matrices[[s]]$ps2,
        adj_matrix = "StARS-refit",
        method = "mb",
        mode = "maxabs",
        label = NULL,
        main_title = paste(s, "Fungi-only (MB)"),
        kingdom = c("Fungi")
      ),
      cross = plot_igraph(
        fit = fits[[s]]$mb$joint_kingdom,
        aligned_matrix1 = aligned_matrices[[s]]$ps1,
        aligned_matrix2 = aligned_matrices[[s]]$ps2,
        adj_matrix = "StARS-refit",
        method = "mb",
        mode = "maxabs",
        label = NULL,
        main_title = paste(s, "Cross-kingdom (MB)"),
        main_title2 = paste(s, "Cross-kingdom (MB)"),
        kingdom = c("Bacteria", "Fungi")
      )
    ),
    glasso = list(
      bact_only = plot_igraph(
        fit = fits[[s]]$glasso$bact_only,
        aligned_matrix1 = aligned_matrices[[s]]$ps1,
        adj_matrix = "StARS-refit",
        method = "glasso",
        mode = "maxabs",
        label = NULL,
        main_title = paste(s, "Bacteria-only (glasso)"),
        kingdom = c("Bacteria")
      ),
      fungi_only = plot_igraph(
        fit = fits[[s]]$glasso$fungi_only,
        aligned_matrix1 = aligned_matrices[[s]]$ps2,
        adj_matrix = "StARS-refit",
        method = "glasso",
        mode = "maxabs",
        label = NULL,
        main_title = paste(s, "Fungi-only (glasso)"),
        kingdom = c("Fungi")
      ),
      cross = plot_igraph(
        fit = fits[[s]]$glasso$joint_kingdom,
        aligned_matrix1 = aligned_matrices[[s]]$ps1,
        aligned_matrix2 = aligned_matrices[[s]]$ps2,
        adj_matrix = "StARS-refit",
        method = "glasso",
        mode = "maxabs",
        label = NULL,
        main_title = paste(s, "Cross-kingdom (glasso)"),
        main_title2 = paste(s, "Cross-kingdom (glasso)"),
        kingdom = c("Bacteria", "Fungi")
      )
    )
  )
